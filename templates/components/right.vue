  ...
  <div id="right">
    ...
    <div class="col-md-3 layout-side" style="float: right">
      ...
      <div class="sidebox">
        ...
        <div class="sidebox-body course-content side-list-body" style="text-align:left" v-if="courseList.length>0">
          <a :href='"/courseDetail?id="+item.id' v-for="item in courseList"><img style="width:25px;height:25px"
                                                                                 :src="item.pic">
            {{item.title}}</a>
        </div>
        <div v-else class="sidebox-body course-content side-list-body" style="text-align:left">
          选择感兴趣的课程
          <a href="javascript:;" v-for="item in tags" v-if="item.id!=99" @click="recommend(item.id)">{{item.name}}</a>
        </div>
        ...
        mounted() {
            if (localStorage.getItem('user_id') != undefined) {
                send('/api/recommend/?user_id=' + localStorage.getItem('user_id'), 'get').then((res) => {
                    ...
                    if (res.data.courseList.length == 0 && localStorage.getItem('tid') == undefined) {
                        send('/api/admin01/tag/', 'get').then((res) => {
                            this.tags = res.data.tags
                        })
                    }
                    if (localStorage.getItem('tid') != undefined) {
                        send('/api/showCourse/?tag=' + localStorage.getItem('tid') + '&sale=-1&member=-1&sort=learn', 'get').then((res) => {
