...
def get_ali_object():
    ...
    # notify_url = "http://127.0.0.1:8000/md_tast/page1_"+id
...
def page1(request):
    if request.method == "GET":
        # 根据当前用户的配置，生成URL，并跳转。
        mes = {}
        memberOrder_id = request.GET.get('memberOrder_id')
        orderRecord_id = request.GET.get('orderRecord_id')
        if memberOrder_id:
            orders = MemberOrder.objects.filter(order_sn=memberOrder_id).first()
